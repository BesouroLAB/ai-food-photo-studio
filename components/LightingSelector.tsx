...
export const LightingSelector: React.FC<LightingSelectorProps> = ({ label, value, options, onChange, onShowDetails, disabled, disabledTooltip, pendingSuggestion }) => {
    ...
    return (
        <div className="mb-4" ref={containerRef} onMouseLeave={handleMouseLeave}>
            <label id="lighting-control-anchor" className="flex items-center text-sm font-medium text-gray-300 mb-2">
                {label}
            ...
            <div className={`grid grid-cols-3 gap-2 ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}>
                {options.map(light => {
                    ...
                    const isSelected = value === light.name;
                    const isSuggested = pendingSuggestion?.payload.setting === 'lighting' && pendingSuggestion?.payload.value === light.name;

                    return (
                        <button
                            key={light.name}
                            onClick={() => !disabled && onChange(light.name)}
                        ...
                        >
                            ...
                            <span className="text-xs text-center font-semibold">{light.name}</span>
                             <div
                                className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity"
                                onClick={(e) => {
                                    e.stopPropagation();
                                    if(!disabled) onShowDetails(light);
                                }}
                            >
